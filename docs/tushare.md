...
必须同时给出 `--start`、`--end`，并在下列两种股票池中选一种（不允许默默拉全市场）：
...
或按指数成分股（取结束日或之前最近一期 `index_weight`）：
...
| `index_weight` | `--index-universe` 成分股 |

`daily` 官方说明：停牌期间不提供数据。只有 `suspend_d` 明确为全日停牌时，才会补 `OHLC=前收、volume=0、is_suspended=true`。未知缺口不会被伪造成停牌。
