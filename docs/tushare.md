...
为处理请求窗口首日附近的全日停牌，拉取器只在原始 `daily` 与 `adj_factor` 请求中保留最多 400 个自然日的前置缓冲，以取得真实前收和复权因子；这些行不会写入最终六表。若前置缓冲内仍没有真实前收且当日官方 `pre_close` 也缺失，命令继续失败，绝不猜测价格。

## 频率与耗时

为避免逐股票请求触发 Tushare `daily` 等接口的 300 次/分钟上限，正式 Tushare 客户端会按接口分别以至少约 `0.31` 秒的间隔发出单代码请求（约 193 次/分钟，留有余量）。离线测试替身不会等待。

因此，像 373 只股票、5 个逐代码接口的历史拉取大约需要 10 分钟以上；执行期间在最终快照写入前可能没有新的终端输出，这是正常的。不要同时启动第二个相同任务。

如果刚收到 `daily` 的频率超限错误，先等待至少 70 秒，让服务端计数窗口归零，再原样重跑命令即可；不需要重新导出股票代码。失败的拉取不会写入或覆盖目标快照。
