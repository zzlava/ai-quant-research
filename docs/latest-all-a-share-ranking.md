...
- 评分时再剔除名称带 `ST` / `*ST` 的证券、全日停牌证券、上市不足 120 个自然日的证券，以及 20 日平均成交额低于 1 亿元的证券。
...
任何一个上市证券的日线、换手、复权或涨跌停记录缺失都会失败；不会把缺失换手率当零，也不会把缺失日线当停牌。当前 `stock_basic` 名称仅用于 resolved `as_of` 当日的 ST 剔除；它不被回填成历史 ST 状态。
